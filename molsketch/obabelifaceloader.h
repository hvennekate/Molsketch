...
public:
  ...
  Molsketch::Molecule* loadFile(const QString& filename, qreal scaling = 1); // TODO check if generating coordinates is necessary
  Molsketch::Molecule* callOsra(const QString filename, qreal scaling = 1);
  bool saveFile(const QString& fileName, const QList<Molsketch::Molecule *> &molecules, bool use3d, bool addHydrogens, qreal scaling); // TODO check if use3d is necessary for that file format
